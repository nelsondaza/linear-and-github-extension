import { IssueSearchPayload, LinearClient } from "@linear/sdk"
import cssText from "data-text:~style.css"
import type { PlasmoCSConfig, PlasmoGetInlineAnchor } from "plasmo"
import { useEffect, useState } from "react"

const LINEAR_PERSONAL_API_KEY =
  process.env.PLASMO_PUBLIC_LINEAR_PERSONAL_API_KEY
...
export const getInlineAnchor: PlasmoGetInlineAnchor = async () => ({
  element: document.querySelector(
    "#partial-discussion-header > div:last-child"
  ),
...
const useTitleCodes = () => {
  const title =
    document.querySelector('#partial-discussion-header h1[class*="title"]')
      ?.textContent || ""
  const codes = title.match(/([A-Z]{2,}-\d+)/gm) || []
...
const useCommentCodes = () => {
  const title =
    document.querySelector('task-lists > div[class*="comment-body"]')
      ?.textContent || ""
  const codes = title.match(/([A-Z]{2,}-\d+)/gm) || []
  console.log({ title, codes })
...
function LinearIssue({ code }: { code: string }) {
  ...
  const [issue, setIssue] = useState<IssueSearchPayload | null | undefined>(
    undefined
  )
